...
La lematización es una técnica de normalización lingüística que consiste en transformar las palabras a su forma base o lema. A diferencia del stemming, que elimina prefijos o sufijos de manera mecánica, la lematización considera el contexto gramatical y produce palabras válidas dentro del idioma.
Por ejemplo:
delivered → deliver
delivering → deliver
deliveries → delivery
La aplicación de lematización permite reducir la variabilidad del vocabulario y mejorar la representación semántica del texto, facilitando posteriores tareas de análisis de sentimiento, clasificación y modelado temático.
...
Además de eliminar stopwords, se incorporó lematización para normalizar las palabras y agrupar distintas variantes morfológicas bajo una representación común. 
...
Posteriormente, se realizó una prueba de tokenización sobre expresiones relacionadas con Amazon Prime para verificar el comportamiento del tokenizador de spaCy. No fue necesario modificar las reglas internas del tokenizer, ya que la segmentación obtenida resultó adecuada para el dominio analizado.
...
Reflexión Crítica
La limpieza textual produjo una reducción del vocabulario del 16,48% y una reducción de tokens del 59,99%, evidenciando un impacto significativo sobre la representación del corpus.
Antes del preprocesamiento predominaban artículos, pronombres y conectores del idioma inglés, además de términos generales asociados al dominio de Amazon. Luego de aplicar stopwords estándar, stopwords personalizadas y lematización, emergieron conceptos directamente vinculados con la experiencia de los usuarios, tales como customer, order, service, delivery, refund y account.
Estos resultados permiten identificar con mayor claridad los temas predominantes presentes en las reseñas y demuestran cómo las técnicas de limpieza textual contribuyen a mejorar la calidad analítica de los datos.
Desde una perspectiva de Ciencia de Datos, la reducción de ruido obtenida facilita futuras tareas de análisis de sentimiento, clasificación automática y modelado predictivo.
...
La tokenización permitió segmentar adecuadamente los documentos, mientras que la La prueba de tokenización realizada permitió verificar el correcto funcionamiento del tokenizador de spaCy sobre textos del dominio analizado. 
...
Como líneas futuras de trabajo podrían incorporarse técnicas complementarias como análisis de sentimiento, representación mediante embeddings y modelos supervisados de clasificación para profundizar el estudio de las opiniones de los clientes. 
